...
const trackNameArr = [
    EnvArrLengthByTrack("trackOne1"),
    EnvArrLengthByTrack("trackOne2"),
    EnvArrLengthByTrack("trackOne3"),
    //you can add as much as you want
];

let envDups = trackNameArr[0];
...
function SwitchEnv(BEAT, TRACK) {
    ...
    for (let i = envIndex; i < envDups; i++) {
        map._customData._customEvents.push(
            {
                "_time": TIME,
                "_type": "AnimateTrack",
    ...
    map._notes = map._notes.filter(x => !x._customData || (x._customData._track !== TRACK));
}
